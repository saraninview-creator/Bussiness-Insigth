...
class FindingsEngine:
        ...
        for col in num_cols:
            ...
            if len(outliers) > 0:
                findings.append({
                    ...
                    "source": "outliers",
                    "text": (
                        f"Found {len(outliers)} highly irregular statistical outliers in '{col}'. "
                        "These massive spikes significantly deviate from standard behavior and require review."
                    ),
                    "meta": {
                        "col": col,
    ...
    def get_categorical_dominance(self) -> list[dict]:
        ...
        for col in cat_cols:
            # Optimize: avoid checking high cardinality ID columns
            ...
            if top_pct > 50:
                findings.append({
                    ...
                    "text": (
                        f"Severe concentration detected: '{top_cat}' dominates the '{col}' category, "
                        f"accounting for {top_pct:.1f}% of all records."
                    ),
                    "meta": {
                        "col": col,
                        "dominant_cat": str(top_cat),
    ...
    def generate(self) -> list[dict]:
        ...
        # Balance outputs dynamically into script segments
        if "21s" in self.explanation_level:
            # Focus entirely on the topmost potent insight + anomaly
            return findings[:2]
            
        # Give us the top 6 distinct narratives
        return findings[:6]
